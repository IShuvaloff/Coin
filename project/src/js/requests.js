  ...
  return fetch('http://localhost:3000/accounts', {
    ...
    .then((res) => {
      if (!res.payload) {
        ...
        // switch (res.error) {
        //   case 'Unauthorized':
        //     result.error = 'Необходима авторизация';
        //     break;
        //   default:
...
// TODO! загрузка координат С ОБОЛОЧКОЙ ДЛЯ ВЫВОДА ОШИБОК для loadCoordinates есть! сделать аналогично для всех остальных!
// ? если выгрузка из БД не удалась, вывести окно с уведомлением, вернуть пустышку нужного типа, иначе вернуть данные
    ...
    return fetch(`${SERVER}/transfer-funds`, {
      ...
      .then((res) => {
        if (!res.payload) {
          ...
          //     result.error = 'Неверный счет отправления';
          //     break;
          //   case 'Invalid account to':
          //     result.error = 'Неверный счет получения';
          //     break;
          //   case 'Invalid amount':
          //     result.error = 'Некорректная сумма перевода';
          //     break;
          //   case 'Overdraft prevented':
          //     result.error = 'Сумма перевода превышает сумму на счету';
          //     break;
          //   default:
          //     result.error = res.error;
          // }
