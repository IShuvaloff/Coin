...
// import { validateAccount } from './validateInputs';
import {
  createBlockAccount,
...
async function onCreateTransactionTo({
  ...
  try {
    ...
    // TODO? 3. перерасчитать динамику баланса
...
async function onExchange({ currencyFromInput, currencyToInput, sumInput }) {
  ...
  // btnExchange.classList.remove('btn-exchange');
  ...
  try {
    ...
    // btnExchange.classList.add('btn-exchange');
  ...
  document.querySelectorAll('.select-check').forEach((select) => {
    ...
    try {
      ...
      if (!target.className?.includes(select.dataset.container)) {
        // ! здесь надо использовать select.id, чтобы закрывать селект при клике вне него
