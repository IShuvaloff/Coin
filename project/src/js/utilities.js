...
function getMonthsTransactionsData(account, monthsCount) {
  ...
  let balance = round(account.balance, 2); // стартовый текущий баланс (будем изменять потранзакционно)
  ...
  let id = 0; // начинаем с месяцем первой записи в инициализированном массиве
  ...
  for (
    ...
    // если месяц транзакции не совпадает с месяцем записи в массиве, переключаемся на следующий месяц и снова проверяем, либо начинаем заполнять
