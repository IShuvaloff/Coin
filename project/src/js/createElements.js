...
function createPagination(accountId, pageCurrent, pagesCount) {
  ...
  //   return document.querySelector('.pagesNums').dataset.pagesCount;
  // }
  // function getPageCurrent() {
  //   return document.querySelector('.pagesNums').dataset.pageCurrent;
  // }
...
function createSelect(name, handlers = {}, items = []) {
  ...
  // TODO! нужно сделать так, чтобы легко было заменять выбранные данные на плейсхолдер (очищать выбранное)

  // eslint-disable-next-line prettier/prettier
  const select = el(`div.select-check.select-${name}__wrapper`, {
    // ! здесь надо уникальный id делать, чтобы закрывать селект при клике вне него
  ...
  // если данных в селекте нет, дизейблить
...
async function createGridAccounts() {
  // если не был выгружен ранее, выгрузить из базы
  if (!accounts?.length) {
...
async function createFormTransactionNew({ accountId }) {
  // загрузить список счетов для селекта в форме
  if (!accounts?.length) {
    const res = await loadAccounts();
    if (!res.success) {
      alert(`Ошибка: ${res?.error || 'неизвестная'}`); // !
  ...
  //   'input.form-transaction__input#input-account-new-transaction'
  // );
  // eslint-disable-next-line prettier/prettier
...
async function createBlockTransactionsHistory({
  ...
  if (history && history.length) {
    // ? формирование информации по страницам
    ...
    // ? создать блок с слайдером страниц, если требуется
...
async function createFormCurrenciesExchange(currenciesList = []) {
  ...
  // const currencyFromInput = el(
  //   'input.form-exchange__input.form-exchange__input--from'
  // );
  ...
  // const currencyToInput = el(
  //   'input.form-exchange__input.form-exchange__input--to'
  // );
